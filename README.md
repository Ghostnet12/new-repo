...
### Search pages and public reviews

Public routes: `/`, `/daily-horoscopes`, `/tarot-deck`, `/birth-chart`, `/tarot-astrology-numerology`, `/reviews`. The client build runs `scripts/build-seo.mjs` to emit page-specific HTML titles/descriptions, canonical URLs, Open Graph/Twitter metadata, WebSite/WebPage JSON-LD and visible introductory content. `shared/pages.js` is shared with the live interface. Real navigation links and browser history preserve the visitor's current form. The legacy daily hash still opens the right section. `/history` is noindex; unknown deployed paths return a real 404. The build also emits `/robots.txt` and `/sitemap.xml`. Submit the sitemap in the owner's verified Google Search Console property; indexing and ranking are not guaranteed by a deploy. No invented ratings or self-serving business review rich-result markup is emitted.

Reviews use the existing MongoDB connection. `GET /api/reviews?page=1` is public and returns 12 reviews plus the count and average; `GET /api/reviews/mine`, `POST /api/reviews`, and `DELETE /api/reviews/mine` require the existing anonymous browser token. One review is stored per browser identity, with up to six create/update operations per hour. This is lightweight abuse reduction, not verified-person authentication. Clearing the browser identity loses the visitor's edit/delete access. Display name, stars, comment and date are public; private readings and birth details are not copied into reviews. Server validation requires explicit publication consent, integer 1–5 stars and a 10–25,000-character comment. Public output excludes the ownership hash and text is rendered through React's escaping. No demo reviews are seeded. Storage failures return an error, never a local-only "published" result. Owner content moderation can currently be performed through the database; there is no new public administrator endpoint.

SEO references: https://developers.google.com/search/docs/crawling-indexing/javascript/javascript-seo-basics and https://developers.google.com/search/docs/crawling-indexing/links-crawlable. Review markup guidance: https://developers.google.com/search/docs/appearance/structured-data/review-snippet.
