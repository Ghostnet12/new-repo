...
export default function App() {
  ...
  useEffect(() => { (async()=>{ try { const [health, deckData] = await Promise.all([api('/api/health'), api('/api/deck')]); setDbReady(health.database === 'connected'); setDeck(deckData.cards); if (health.database === 'connected') { const h = await api('/api/readings?limit=12'); setHistory(h.readings); const p = await api('/api/profile').catch(()=>null); if (p?.profile) setForm(f => ({...f, ...p.profile, spread:p.profile.preferredSpread || f.spread})); } } catch(e){ setError(e.message); } })(); }, []);
  const generate = async () => { setLoading(true); setError(''); try { const result = await api('/api/readings/generate',{method:'POST',body:JSON.stringify({profile:{name:form.name,gender:form.gender,birthday:form.birthday,preferredSpread:form.spread}, question:form.question, spread:form.spread, focus:form.focus, need:form.need, reversals:form.reversals==='yes'})}); setReading(result); setDbReady(result.database === 'connected'); if (result.persisted) { const h=await api('/api/readings?limit=12'); setHistory(h.readings); } setTab('read'); } catch(e){setError(e.message);} finally{setLoading(false);} };
