...
export default function NatalForm({value,onChange,includeBirthday=false}) {
 ...
 return <fieldset className="natal-inputs"><legend>Birth chart details</legend>
  ...
  {natal.enabled&&<>
   ...
   <label><span>Search birthplace</span><input value={query} onChange={e=>{setQuery(e.target.value);setResults([]);setMessage('');}} placeholder="City, region or country" onKeyDown={e=>{if(e.key==='Enter'){e.preventDefault();search();}}}/></label>
   ...
   <details><summary>Location & clock-change details</summary><p className="symbolism-note">Search uses city-center coordinates. You can enter more precise coordinates and the birthplace’s IANA time zone here. Historical daylight-saving offsets are applied to your date.</p>
    <label><span>Birthplace label</span><input value={natal.place} maxLength={160} onChange={e=>set('place',e.target.value)}/></label>
    ...
    <label><span>Time zone</span><input value={natal.timezone} maxLength={80} placeholder="America/Chicago" onChange={e=>set('timezone',e.target.value)}/></label>
