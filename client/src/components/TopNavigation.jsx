...
export default function TopNavigation({ items, activeId, onChoose, onHome, onProfile, onDeck }) {
  ...
  return <header className="mystic-topbar" ref={rootRef} onBlur={event => {
    ...
    <div className="topbar-inner">
      <button className="mystic-brand" onClick={() => { setOpen(false); onHome(); }} aria-label="The Fold home">
      ...
      {open && <nav id="fold-menu" className="fold-menu" ref={menuRef} aria-label="Explore The Fold">
        ...
        {items.map(item => <button key={item.id} aria-current={activeId === item.id ? 'page' : undefined} onClick={() => choose(item)}><NavIcon name={item.id} /><span>{item.label}</span>{activeId === item.id && <span className="menu-current">Current</span>}</button>)}
        <button className="menu-deck" onClick={() => { setOpen(false); onDeck(); }}><NavIcon name="read" /><span>Explore the full deck</span><NavIcon name="arrow" /></button>
      </nav>}
    </div>
  </header>;
}
