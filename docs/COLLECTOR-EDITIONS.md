...
4. The first successful collector status request initialises the fixed first-series manifest in the configured MongoDB replica-set database. It never resets existing stock and accepts no caller-supplied content. To initialise explicitly from an operator environment, use `node scripts/seed-collector-edition.mjs --confirm-first-series` with the server’s `MONGODB_URI`. This command is idempotent and never reshuffles an existing edition. Seed a separate test database for sandbox testing. Never mix test purchases with the live inventory.
