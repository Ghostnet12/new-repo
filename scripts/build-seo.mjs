...
for(const page of Object.values(pages)){
 ...
 const fallback=`<main><h1>${esc(page.heading)}</h1><p>${esc(page.text)}</p>${page.path==='/'?`<section><h2>Your tarot questions, answered.</h2>${readingFaqs.map(item=>`<details><summary>${esc(item.question)}</summary><p>${esc(item.answer)}</p></details>`).join('')}</section>`:''}<nav aria-label="Explore The Fold">${Object.values(pages).map(p=>`<p><a href="${p.path}">${esc(p.label)}</a></p>`).join('')}</nav><p>${page.path==='/fortune-teller'?'Enable JavaScript to reveal and save your fortune card.':'Enable JavaScript to use the interactive readings and review form.'}</p></main>`;
 const pageTemplate=page.path==='/fortune-teller'?template.replace('href="/assets/the-fold/hero-clean.png"','href="/assets/the-fold/fortune-teller.webp"'):template;
