...
for(const page of Object.values(pages)){
 ...
 const fallback=`<main><h1>${esc(page.heading)}</h1><p>${esc(page.text)}</p>${page.path==='/'?`<section><h2>Your tarot questions, answered.</h2>${readingFaqs.map(item=>`<details><summary>${esc(item.question)}</summary><p>${esc(item.answer)}</p></details>`).join('')}</section>`:''}<nav aria-label="Explore The Fold">${Object.values(pages).map(p=>`<p><a href="${p.path}">${esc(p.label)}</a></p>`).join('')}</nav><p>Enable JavaScript to use the interactive readings and review form.</p></main>`;
